...
;(function () {
  ...
  function createLine(line, printer, state) {
    ...
    if ('code' in column) {
      ...
      if (column.code.type === 'qrcode') {
        ...
        result.push(
          printer.command.normal() + printer.command.area(left, width, right) + printer.command.align(column.align) + printer.command.qrcode(column.code, printer.encoding),
        )
      }
      // process barcode
      else {
        ...
        result.push(
          printer.command.normal() + printer.command.area(left, width, right) + printer.command.align(column.align) + printer.command.barcode(column.code, printer.encoding),
        )
      }
    }
    // process command
  ...
  const _svg = {
    ...
    vrstart: function (widths) {
      ...
      const path =
        `<path d="` +
        widths
          .reduce((a, width) => a + `h${w * width}h${u}v${w}m0,${-w}h${u}`, `M${u},${w + w}v${-u}q0,${-u},${u},${-u}`)
    ...
    vrstop: function (widths) {
      ...
      const path =
        `<path d="` +
        widths.reduce((a, width) => a + `h${w * width}h${u}v${-w}m0,${w}h${u}`, `M${u},0v${u}q0,${u},${u},${u}`).replace(/h\d+v-\d+m0,\d+h\d+$/, `q${u},0,${u},${-u}v${-u}`) +
        `" fill="none" stroke="#000" stroke-width="2"/>`
      this.svgContent += `<g transform="translate(${this.lineMargin * w},${this.svgHeight})">${path}</g>`
      return ''
    },
    ...
    vrhr: function (widths1, widths2, dl, dr) {
      ...
      const path1 =
        `<path d="` +
        widths1
          .reduce((a, width) => a + `h${w * width}h${u}v${-w}m0,${w}h${u}`, `M${u},0` + (dl > 0 ? `v${u}q0,${u},${u},${u}` : `v${w}h${u}`))
          .replace(/h\d+v-\d+m0,\d+h\d+$/, dr < 0 ? `q${u},0,${u},${-u}v${-u}` : `h${u}v${-w}`) +
        `" fill="none" stroke="#000" stroke-width="2"/>`
      this.svgContent += `<g transform="translate(${(this.lineMargin + Math.max(-dl, 0)) * w},${this.svgHeight})">${path1}</g>`
      const path2 =
        `<path d="` +
        widths2
          .reduce((a, width) => a + `h${w * width}h${u}v${w}m0,${-w}h${u}`, `M${u},${w + w}` + (dl < 0 ? `v${-u}q0,${-u},${u},${-u}` : `v${-w}h${u}`))
          .replace(/h\d+v\d+m0,-\d+h\d+$/, dr > 0 ? `q${u},0,${u},${u}v${u}` : `h${u}v${w}`) +
        `" fill="none" stroke="#000" stroke-width="2"/>`
      ...
      if ('module' in bar) {
        ...
        if (symbol.hri) {
          ...
          const tspan = bar.hri
            .split('')
            .reduce(
              (a, c, i) => a + `<tspan x="${m + this.charWidth * i}">${c.replace(/[ &<>]/g, (r) => ({ ' ': '&#xa0;', '&': '&amp;', '<': '&lt;', '>': '&gt;' }[r]))}</tspan>`,
              '',
            )
  ...
  const _thermal = {
    ...
    open: function (printer) {
      ...
      return (
        ...
        // 10-01 Ynewtime
        // Disable the following two lines can support both CN710-UE and Micro T220-MD models:
        // 10-02 Ynewtime
        // Don't modify the default commands, instead you can use your own commands:
        // https://github.com/receiptline/receiptline/blob/master/example/command/custom.js
        '\x1c(A' +
        $(2, 0, 48, 0) +
    ...
    // 10-01 Ynewtime
    // 10-02 Ynewtime: don't modify default commands
    ...
    vrhr: function (widths1, widths2, dl, dr) {
      const r1 =
        ...
        widths1.reduce((a, w) => a + '\x95'.repeat(w) + '\x90', dl > 0 ? '\x9e' : '\x9a').slice(0, -1) +
        (dr < 0 ? '\x9f' : '\x9b') +
        ' '.repeat(Math.max(dr, 0))
      const r2 =
        ' '.repeat(Math.max(dl, 0)) +
        widths2.reduce((a, w) => a + '\x95'.repeat(w) + '\x91', dl < 0 ? '\x9c' : '\x98').slice(0, -1) +
        (dr > 0 ? '\x9d' : '\x99') +
        ' '.repeat(Math.max(-dr, 0))
        ...
        for (let y = 0; y < h; y++) {
          ...
          for (let x = 0; x < w; x += 8) {
            ...
            for (let p = 0; p < q; p++) {
              const f = Math.floor(
                (d[i] + e * 5) / 16 +
                  Math.pow(((img.data[j] * 0.299 + img.data[j + 1] * 0.587 + img.data[j + 2] * 0.114 - 255) * img.data[j + 3] + 65525) / 65525, 1 / this.gamma) * 255,
              )
  ...
  const _impact = {
    ...
    hr: function (width) {
      return '\x1b!' + $(this.font) + ' '.repeat(this.margin) + '\x1bt\x00' + '\x95'.repeat(width)
    },
