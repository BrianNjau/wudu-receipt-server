 ...
 * @param {{ extend: import('receiptline').Printer, command: import('receiptline').BaseCommand }} [custom] custom printer commands
 * @returns {string} print result, printer status, or print image
 */
const print = (receiptmd, options, custom = {}) => {
  const { extend, command } = custom
  return new Promise(async (resolve) => {
    // options
    const params = parseOption(options)
    const printer = convertOption(params, command)
    // print timeout
...
// 10-02 Ynewtime
// use custom command to support other printer models
const convertOption = (params, command) => {
  // language
...
const rasterize = async (receiptmd, printer, encoding) => {
  ...
  if (puppeteer) {
    ...
    const png = await page.screenshot({ encoding, omitBackground: true })
    await browser.close()
    return png
  } else if (sharp) {
...
module.exports = { print, createPrint }
