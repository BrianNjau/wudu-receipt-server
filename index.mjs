  ...
  async function go(session, body, res) {
    try {
      ...
      if (!(toPrintBillContent && toPrintBillContent.length) && !(toPrintOrderContent && toPrintOrderContent.length) && !(toPrintRefundContent && toPrintRefundContent.length) && !(toPrintRevenueAnalysisContent && toPrintRevenueAnalysisContent.length)) {
        return handler('1', `Print failed: 'toPrintBillContent', 'toPrintOrderContent' , 'toPrintRevenueAnalysisContent' and 'toPrintRefundContent' empty.`)
      }
            ...
            for (const record of toPrintRevenueAnalysisContent) {
              try {
                ...
                if (hardwareType === 'Network') {
                  if (!ip) handler('1', `Print bill:${receiptType}|${billInfo} to Network failed: ip empty.`)
                  else if (net.isIP(ip) !== 4) handler('1', `Print bill:${receiptType}|${billInfo} to Network failed: ip:${ip} incorrect, should be IPv4 format like: 1.1.1.1.`)
                  else {
                    ping.sys.probe(ip, async function (isAlive) {
                      if (!isAlive) handler('1', `Print bill:${receiptType}|${billInfo} to Network failed: ip:${ip} failed to connect.`)
                      else {
                        await print(buildRevenueAnalysis(revenueAnalysis), `-d ${ip} -l zh`)
                        handler('0', `Print bill:${receiptType}|${billInfo} to Network:${ip} success.`)
                      }
                    })
                  }
                } else if (hardwareType === 'USB') {
                  if (!hasUsbPrinters) handler('1', `Print bill:${receiptType}|${billInfo} to USB:${vid}|${pid} failed: USB Printers Not Found`)
                  else {
                    const commands = await print(buildRevenueAnalysis(revenueAnalysis), `-l zh`)
                    const device = new USB(vid, pid)
                    device.open((err) => {
                      if (err) handler('1', `Print bill:${receiptType}|${billInfo} to USB:${vid}|${pid} failed|USB device open failed: ${err}.`)
                      else {
                        device.write(Buffer.from(commands, 'binary'), async (writeErr) => {
                          if (writeErr) handler('1', `Print bill:${receiptType}|${billInfo} to USB:${vid}|${pid} failed|USB device write failed: ${writeErr}.`, () => device.close(next))
                          else {
                            const waitTime = printTimeMap[pid]
                            await sleep(waitTime)
                            handler('0', `Print bill:${receiptType}|${billInfo} to USB:${vid}|${pid} success.`, () => device.close(next))
                          }
                        })
                      }
                    })
                  }
                } else if (OTHER_BRAND.includes(hardwareType)) handler('0', `Print bill:${receiptType}|${billInfo}: ignore hardwareType ${hardwareType}`)
                else handler('1', `Print bill:${receiptType}|${billInfo} failed: Unsupported hardwareType: ${hardwareType}`)
              } catch (err) {
                handler('1', `Print Rev Analysis failed: ${err.message}`)
              }
            }
          }
